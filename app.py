...
def index():
    ...
    car_models = sorted(car_df['name'].unique())
    year = sorted(car_df['year'].unique(), reverse=True)
...
def predict():
    ...
    prediction = regmodel.predict(pd.DataFrame(columns=['name', 'company', 'year', 'kms_driven', 'fuel_type'],
